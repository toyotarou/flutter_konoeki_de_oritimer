...
| `'geofence_pending_alert'` | `bool` | `geofenceCallback` 発火時 | 停止操作時（`_stopFirstGoal()` / `_removeAllGeofences()`）|
...
**初期化処理（initState）:**
- `WidgetsBinding.instance.addObserver(this)` を登録（ライフサイクル監視）
- `_initPlugins()` と `_loadMultiGoals()` を呼び出す:
  ...
  7. `WidgetsBinding.instance.addPostFrameCallback(() => _checkAndShowPendingGeofenceAlert())` → 起動前のジオフェンス発火フラグを確認してダイアログを表示する

**dispose 処理:**
- `WidgetsBinding.instance.removeObserver(this)` を呼び出してオブザーバーを解除する

**AppLifecycleState 監視（`didChangeAppLifecycleState`）:**
- `AppLifecycleState.resumed` 時に `_checkAndShowPendingGeofenceAlert()` を呼び出す
- バックグラウンドでジオフェンスが発火した後、アプリをフォアグラウンドへ戻したときにダイアログが表示される

**`_checkAndShowPendingGeofenceAlert()` の仕様:**
```dart
...
```
- `geofence_pending_alert` フラグが `true` かつ、ダイアログ未表示の場合のみダイアログを表示する
- `_showGeofenceAlertDialog()` は既存の実装（multiGoalMap / selected を参照して駅名を決定）をそのまま使用するため、単体・マルチゴール両方に対応している
...
Future<void> geofenceCallback(GeofenceCallbackParams params) async {
  ...
  // 5. ジオフェンス発火済みフラグを保存する（バックグラウンド起動後の復元用）
  //    アプリが起動・フォアグラウンド復帰した際にこのフラグを検出してダイアログを表示する
  await SharedPreferencesService.saveGeofencePendingAlert();

  // 6. アプリが前面にある場合、UI isolate へ直接通知してダイアログを表示させる
...
| `kGeofencePendingAlert` | `'geofence_pending_alert'` | ジオフェンス発火済みフラグ（バックグラウンド発火後のダイアログ表示用） |
...
| `saveGeofencePendingAlert()` | なし | `Future<void>` | ジオフェンス発火済みフラグを true で保存する（`geofenceCallback` から呼び出す） |
| `loadGeofencePendingAlert()` | なし | `Future<bool>` | ジオフェンス発火済みフラグを読み込む（未設定は false） |
| `clearGeofencePendingAlert()` | なし | `Future<void>` | ジオフェンス発火済みフラグを削除する（停止操作時に呼び出す） |
...
| 1.10 | 2026-04-05 | バックグラウンド/終了状態でのジオフェンス発火後、アプリ起動・フォアグラウンド復帰時にアラートダイアログを表示する機能を追加。`geofenceCallback` に `SharedPreferencesService.saveGeofencePendingAlert()` 呼び出しを追加（6.3）。SharedPreferencesService に `kGeofencePendingAlert` キー定数と `saveGeofencePendingAlert()`・`loadGeofencePendingAlert()`・`clearGeofencePendingAlert()` メソッドを追加（6.6）。HomeScreen に `WidgetsBindingObserver` ミックスイン・`didChangeAppLifecycleState()`・`_checkAndShowPendingGeofenceAlert()` を追加（6.2）。`_removeAllGeofences()` にフラグクリア処理を追加（6.2）。SharedPreferences キー表に `geofence_pending_alert` を追加（4.3） |
